...
public class WordCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Table t = tEnv.fromDataStream(s1).as("word", "cnt");
//        t.printSchema();
        // t.select().groupBy()
//        t.groupBy($("word")).select($("word"), $("cnt").sum().as("cnts")).execute().print();
...
//                .columnByMetadata("rt", DataTypes.TIMESTAMP_LTZ(3), "proctime()")    // 从底层数据源（流，或者connector）中抽取某个元数据，声明成一个表的逻辑字段
                .build();

        tEnv.createTemporaryView("t4", s1, schema);
